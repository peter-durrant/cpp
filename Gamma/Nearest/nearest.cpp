...
Nearest::Nearest(uint q, const kdTree& k, uint p)
    : query_point(k[q]), kdtree(k), pmax(p), near_neighbours(pmax), zero_nn_list(0), furthest_distance(fp_MAX),
...
Nearest :: ~Nearest(void)
{
    for (uint i = 0; i < near_neighbours.size(); i++) {
        if (near_neighbours[i]) delete near_neighbours[i];
        near_neighbours[i] = 0;
    }
}
...
/*bool Nearest :: Ball_Within_Bounds(void)
{
...
bool Nearest::Bounds_Overlap_Ball(void)
{
    fp sum = fp_ZERO;
    const valarray_fp& query_input = query_point.Input_Vector();
    for (uint d = 0; d < kdtree.Dimension(); d++) {
        if (query_input[d] < lower_bound[d]) { // lower than low boundary
            ...
            if (sum > furthest_distance) return false; // squared distances
...
const std::vector<near_points*>& Nearest::neighbours(void) const
{
